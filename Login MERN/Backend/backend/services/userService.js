...
const userRegisterService = async (req, res) => {
    const { firstname, lastname, email, password, pincode, gender, city, state,isActive,userType } = req.body;

    if (!firstname || !lastname || !email || !password || !pincode || !gender) {
    ...
    try {
        ...
        await newUser.save();
        return res.status(201).json({ success: true, msg: "User successfully registered" });
     } catch (error) {
...
module.exports = { userRegisterService, userLoginService, userData,userDelete,userEdit,isActiveUser};
